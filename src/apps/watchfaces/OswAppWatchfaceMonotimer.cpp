 ...
 */
void OswAppWatchfaceMonotimer::drawNShiftedTicks(Graphics2D* gfx, uint8_t cx, uint8_t cy, uint8_t r1, uint8_t r2, 
                                                 uint8_t nTicks, float shift, uint16_t color, bool anti_alias) {
    float deltaAngle = 360.0f / nTicks;
    for (uint16_t i=0; i<nTicks; ++i) {
        gfx->drawTick(cx, cy, r1, r2, (i * deltaAngle) + shift, color, anti_alias);
    }
}
 ...
 * @param color color code
 */
void OswAppWatchfaceMonotimer::drawNShiftedMaskedTicks(Graphics2D* gfx, uint8_t cx, uint8_t cy, uint8_t r1, uint8_t r2, uint8_t nTicks, 
                                                       float shift, uint16_t m, uint16_t color, bool anti_alias) {
    float deltaAngle = 360.0f / nTicks;
    for (uint16_t i=0; i<nTicks; ++i) {
        if (i % m) {
            gfx->drawTick(cx, cy, r1, r2, (i * deltaAngle) + shift, color, anti_alias);
...
void OswAppWatchfaceMonotimer::drawWatch() {
    ...
    // hours
    hal->gfx()->drawNTicks(120, 120, 117, 100, 12, ui->getForegroundColor(), 361, true);
    // 30 minutes
    drawNShiftedTicks(hal->gfx(), 120, 120, 117, 105, 12, 360.0f/24.0f, ui->getForegroundColor(), true);
    // 15 minutes
    drawNShiftedTicks(hal->gfx(), 120, 120, 110, 105, 24, 360.0f/48.0f, ui->getForegroundColor(), true);
    // 5 minutes
    drawNShiftedMaskedTicks(hal->gfx(), 120, 120, 109, 108, 144, 0.0f, 3, ui->getForegroundColor(), true);
    ...
    if (OswConfigAllKeys::settingDisplayDualHourTick.get()) {
        ...
        hal->gfx()->drawThickTick(120, 120, 0, 105, (360.0f * (60 * dualHour + dualMinute)) / 720.0f, 3, ui->getBackgroundDimmedColor(), true);
    }

    hal->gfx()->drawThickTick(120, 120, 0, 105, (360.0f * (60 * hour + minute)) / 720.0f, 3, ui->getForegroundColor(), true, ROUND_END);

    hal->gfx()->fillCircleAA(120, 120, 6, ui->getForegroundColor());
}
